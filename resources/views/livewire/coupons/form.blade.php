...
$create = function(){
    ...
    $data['image'] = $data['image']->store('coupon-photos/' . $authUser->listing->public_id, 'public');
...
<div >
    ...
    <div class="space-y-4 mt-4">
        @if($this->type == 'update')
            <div x-data="{ edit: false }">
                <div x-show="edit">
                    ...
                    <div @click="edit = false" class="py-4 hover:text-blue-500 cursor-pointer">Show Current Image</div>
                </div>
                <div x-show="!edit" @dblClick="edit = true">
                    <div class="text-xs">Double click image to edit</div>
                    <img src="{{ asset($coupon->image )}}" class=" rounded aspect-video w-64 object-cover" />   
                </div>
            </div>
        @else
            <flux:input wire:model="image" label="Image" type="file" />
        ...
        <flux:input wire:model="description" label="Description" placeholder="What does your customer get with this coupon?" />
