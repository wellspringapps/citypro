...
on(['message-archived' => function() {
    unset($this->submissions);
}]);

$submissions = computed(function(){
    return auth()->user()->listing->contactSubmissions()->orderBy('created_at', 'desc')->paginate(10);
});
            ...
            <flux:rows>
                @foreach ($this->submissions as $submission)
                    <flux:row :key="$submission->id">
                        ...
                        <flux:cell class="whitespace-nowrap">{{ $submission->created_at->format('m/j/y') }}</flux:cell>
